...
public class EventoManagementPortlet extends MVCPortlet {
	private static SimpleDateFormat sdfQueryDate = new SimpleDateFormat(
			"yyyy-MM-dd");
	...
	public void serveResource(ResourceRequest resourceRequest,
		...
		String startRange = ParamUtil.getString(resourceRequest, "start");
		String endRange = ParamUtil.getString(resourceRequest, "end");

		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");

		List<CalendarBooking> eventList = null;
		try {
			eventList = getEventsByDateRange(dateFormat.parse(startRange)
					.getTime(), dateFormat.parse(endRange).getTime());
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//					dateFormatter.format(cal.getTime())));
			...
			try {
				String baseUrl = Utils.getBaseURL(PortalUtil
...
//		String events = stringBuf.toString();
	...
	public void doView(RenderRequest renderRequest,
		...
		nextDateCalendar.set(Calendar.HOUR_OF_DAY, 0); // ! clear would not
														// reset the hour of day
														// !
		...
		prevDateCalendar.set(Calendar.HOUR_OF_DAY, 0); // ! clear would not
														// reset the hour of day
														// !
		...
		// title data
		SimpleDateFormat sdfTitleDate = new SimpleDateFormat("MMMM yyyy",
				locale);
	...
	public List<CalendarBooking> getEventsByCallId(Date queryDate, long callId)
		...
		DynamicQuery dynamicQuery = CalendarBookingLocalServiceUtil
				.dynamicQuery();
		Criterion criterionGroup = RestrictionsFactoryUtil.eq("groupId",
				groupId);
		...
		dynamicQuery.add(RestrictionsFactoryUtil.and(criterionTime,
				RestrictionsFactoryUtil.and(criterionGroup, criterionStatus)));
	...
	@SuppressWarnings("unchecked")
	public List<CalendarBooking> getEventsByDateRange(long start, long end)
			throws SystemException {
		List<CalendarBooking> eventList;
		DynamicQuery dynamicQuery = CalendarBookingLocalServiceUtil
				.dynamicQuery();
	...
	@SuppressWarnings("unchecked")
	public List<CalendarBooking> getEventsByCategoryId(Date queryDate,
		...
		// RestrictionsFactoryUtil.in("calendarBookingId", ids);
		Long[] range = getTimeInterval(queryDate);
		Criterion criterionTime = RestrictionsFactoryUtil.between("startTime",
				range[0], range[1]);
		dynamicQuery.add(RestrictionsFactoryUtil.and(criterionTime,
		...
		List<CalendarBooking> queryEventList = (List<CalendarBooking>) CalendarBookingLocalServiceUtil
				.dynamicQuery(dynamicQuery);
	...
	@SuppressWarnings("unchecked")
	public List<CalendarBooking> getEventsByIdeaId(Date queryDate, long ideaId)
			throws SystemException, PortalException {
		List<CalendarBooking> eventList;
	...
	public void updateEvent(ActionRequest request, ActionResponse response)
		...
		if (Validator.isNotNull(eventId)) {
			...
			// sdf.format(calendarStart.getTime()));
			...
			if (calendarStart.getTimeInMillis() >= calendarEnd
					.getTimeInMillis()) {
				...
				response.setRenderParameter("jspPage",
						"/html/eventomanagement/edit.jsp");
			} else {
				CalendarBooking event = CalendarBookingLocalServiceUtil
						.getCalendarBooking(eventId);
	...
	public void addEvent(ActionRequest request, ActionResponse response)
		...
		if (calendarStart.getTimeInMillis() >= calendarEnd.getTimeInMillis()) {
			...
			response.setRenderParameter("jspPage",
					"/html/eventomanagement/add.jsp");
		} else {
			ServiceContext serviceContext = ServiceContextFactory.getInstance(
					CalendarBooking.class.getName(), request);
			// get calendar by groupId
			DynamicQuery dynamicQuery = CalendarLocalServiceUtil.dynamicQuery();
			Criterion criterionGroup = RestrictionsFactoryUtil.eq("groupId",
					groupId);
			dynamicQuery.add(criterionGroup);
			List<com.liferay.calendar.model.Calendar> calendarList = CalendarLocalServiceUtil
					.dynamicQuery(dynamicQuery);
			if (!calendarList.isEmpty()) {
				...
				Map<Locale, String> descriptionMap = new HashMap<>();
				descriptionMap.put(locale, description);
				com.liferay.calendar.model.Calendar calendar = calendarList
						.get(0);
				CalendarBooking event = CalendarBookingLocalServiceUtil
						.addCalendarBooking(userId, calendar.getCalendarId(),
								new long[] {}, 0L, titleMap, descriptionMap,
								null, calendarStart.getTimeInMillis(),
								calendarEnd.getTimeInMillis(), false, null, 0L,
								null, 0L, null, serviceContext);
				if (Validator.isNotNull(location)) {
					event.setLocation(location);
				}
				...
				AssetEntryLocalServiceUtil.updateEntry(userId, groupId,
						CalendarBooking.class.getName(),
						event.getCalendarBookingId(), categoryIds,
						new String[] {});
			}
		}
	}
	...
	private void getSearchIteratorData(RenderRequest renderRequest,
			RenderResponse renderResponse, List<CalendarBooking> eventList) {
		// search-iterator tag
		...
		SearchContainer<CalendarBooking> searchEvents = new SearchContainer<CalendarBooking>(
				renderRequest, null, null, SearchContainer.DEFAULT_CUR_PARAM,
				5, portletURL, headersEvent, "Empty resultset");
		...
		SearchContainer<CalendarResource> searchResources = new SearchContainer<CalendarResource>(
				renderRequest, null, null, SearchContainer.DEFAULT_CUR_PARAM,
				5, portletURL, headersResource, "Empty resultset");
		searchResources.setTotal(eventList.size());

		for (int i = searchEvents.getStart(); i < searchEvents.getEnd()
				&& i < searchEvents.getTotal(); i++) {
			CalendarBooking event = eventList.get(i);
			try {
				ResultRow rowEvent = new ResultRow(event,
						event.getCalendarBookingId(), i, false);
				...
				rowEvent.addText(Long.toString(event.getCalendar()
						.getCalendarId()));
				rowEvent.addText(Long.toString(event.getCalendar()
				...
				ResultRow rowResource = new ResultRow(resource,
						resource.getCalendarResourceId(), i, false);
				rowResource.addText(Long.toString(resource
						.getCalendarResourceId()));

  ...
  <c:otherwise>
    ...
    <aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="fm">
      ...
      <aui:button-row>
        ...
        <liferay-util:include page='<%= "/html/portlet/login/navigation/facebook.jsp" %>' portletId="<%= portletDisplay.getRootPortletId() %>" />
        <liferay-util:include page='<%= "/html/portlet/login/navigation/google.jsp" %>' portletId="<%= portletDisplay.getRootPortletId() %>" />
      </aui:button-row>
    </aui:form>

...
<aui:form action="<%= editFileEntryURL %>" cssClass="lfr-dynamic-form" enctype="multipart/form-data" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveFileEntry(" + saveAsDraft + ");" %>'>
	...
	<aui:fieldset>
		...
		<aui:field-wrapper cssClass="folder-wrapper" label="folder">
      ...
      <aui:button cssClass="formbutton-cancel" href="<%= redirect %>" type="cancel" />
			...
			<aui:button cssClass="formbutton-primary" disabled="<%= checkedOut && !hasLock || (pending && PropsValues.DL_FILE_ENTRY_DRAFTS_ENABLED) %>" name="publishButton" type="submit" value="<%= publishButtonLabel %>" />
